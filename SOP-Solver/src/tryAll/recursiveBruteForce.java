...
/**
 * Finds the perfect Solution for a given SOP-Instance. Implements the
 * Brute-Force-Method via recursion. The instance has to have the first node as
 * fixed starting point and the last node as fixed destination. Therefore
 * supports the SOP-Instances of the TSPLib.
 * 
...
public class recursiveBruteForce {
	...
	/**
	 * Finds the perfect Solution for a given SOP-Instance via the
	 * Brute-Force-Method. Calculates every possible path and picks the shortest
	 * one. Calls the recursive method "recursion".
	 * 
	 * @param weights
	 *            the given SOP-Instance of the TSP-Lib as matrix of
	 *            edge-lengths (weights) and dependencies.
	 * @return the perfect result as List of node-numbers. Does not contain the
	 *         starting node (which is always the first) and the destination
	 *         (which is always the last node). They will be added by the
	 *         Main-Class. Therefore Nodes beginning with 2.
	 */
	 ...
	 * the path. If the path is complete, checks the Length of it. If it is the
	 * current Minimum, saves the path and the length.
	 * 
	 * @param curPath
	 ...
	 * it. All dependencies for the node are fulfilled, if every node that has
	 * to be visited before the node to check are already in the path. If node i
	 * has to be visited before the node, matrix[node][i] is -1. Also ensures,
	 * that the first node is always the start of a path and the last node
	 ...
	 * 
	 * @param curPath
	 *            The path that bestPath should be when this method terminates.
	 */
